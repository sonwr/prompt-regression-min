...
def _build_reviewer_queue(summary: dict[str, object]) -> dict[str, object]:
    ...
    runner_up_key = None if second_focus_group is None else str(second_focus_group["key"])
    runner_up_label = None if second_focus_group is None else str(second_focus_group["label"])
    runner_up_ids = [] if second_focus_group is None else list(second_focus_group["ids"])
    ...
    next_focus_advantage_summary = (
        ...
        if next_focus_group is None
            ...
            f"{next_focus_advantage_label}: +{next_focus_advantage_case_count} case(s), "
            f"+{next_focus_advantage_queue_share * 100:.2f}% queue share, "
            f"+{next_focus_advantage_active_case_rate * 100:.2f}% active-case rate, "
            f"+{next_focus_advantage_source_case_rate * 100:.2f}% source-case rate"
        )
    )
    next_focus_tie_keys = [
        str(group["key"])
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    run_cmd.add_argument(
        "--summary-markdown", "--summary-md", "--summary-pr-md", "--summary-status-md", "--summary-release-md", "--summary-owner-md", "--summary-lead-md", "--summary-report-md",
        default=None,
        help=(
