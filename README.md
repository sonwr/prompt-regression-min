...
If you need a compact repo-4/repo-5 reminder that the same short status can call out `report_output_title` when governance-sandbox report basenames come from scenario files, open `docs/CLI_SUMMARY_REPO45_REPORT_OUTPUT_TITLE_ALIAS_NOTE.md`.
If you need a compact repo-4/repo-5 reminder that the same short status can call out `report_output_basename` when governance-sandbox report basenames come from scenario files, open `docs/CLI_SUMMARY_REPO45_REPORT_OUTPUT_BASENAME_ALIAS_NOTE.md`.
...
If you need the matching compact reminder that the same five-line report should keep repo name, change state, validation, and push/hold status in one Korean-ready line, open `docs/CLI_SUMMARY_FIVE_REPO_KOREAN_STATUS_STACK_NOTE.md`.
...
If you need a compact reminder that the five-line summary still keeps repo 4 and repo 5 visible every run, open `docs/CLI_SUMMARY_FIVE_LINE_REPO45_ALWAYS_PRESENT_NOTE.md`.
...
If you need a compact reminder that a saved summary bundle should expose one owner-visible validation gate before commit/push, open `docs/CLI_SUMMARY_VALIDATION_GATE_OWNER_NOTE.md`.
...
If you need a compact reviewer-facing example that keeps queue focus, owner, and `report_bundle_tag` aligned in one saved summary bundle, open `examples/reviewer_queue_report_bundle_tag_note.md`.
...
If you need the shortest reminder that each repo line should only claim validated progress, open `docs/CLI_SUMMARY_VALIDATED_FIVE_REPO_ONE_LINE_NOTE.md`.
...
If you need a compact reminder that those five cron-facing lines should stay short and Korean for the user-facing handoff, open `docs/CLI_SUMMARY_FIVE_REPO_KOREAN_SHORT_LINE_NOTE.md`.
If you need the matching reminder that the same five-repo handoff should stay plain text and exactly five lines, open `docs/CLI_SUMMARY_PLAIN_TEXT_FIVE_LINE_NOTE.md`.
If you need a compact reminder to keep each of those five lines ordered as repo name -> change state -> validation -> commit/push or hold reason, open `docs/CLI_SUMMARY_FIVE_REPO_FIELD_ORDER_NOTE.md`.
...
If you need a compact reminder that the same five-line repo report should keep each hold reason specific and short, open `docs/CLI_SUMMARY_FIVE_REPO_HOLD_REASON_TRIM_NOTE.md`.
...
If you need a compact repo-4/repo-5 cue for the same short status when governance artifact paths are grouped under `report.outputs.files`, open `docs/CLI_SUMMARY_REPO45_REPORT_OUTPUT_FILES_STATUS_NOTE.md`.
...
If you need the matching repo-4/repo-5 cue for `scenario_journal` or `scenario_journal_bundle` wrapper proof plus the same short governance report line, open `docs/CLI_SUMMARY_REPO45_SCENARIO_JOURNAL_REPORT_NOTE.md`.
...
If you need a compact reminder to drive the saved JSON/Markdown/HTML trio from one deterministic output slug, open `docs/CLI_SUMMARY_REPORT_OUTPUT_SLUG_TRIO_NOTE.md`.
...
If you need a compact reminder to state the exact commit/push hold reason in that same five-repo line, open `docs/CLI_SUMMARY_FIVE_REPO_COMMIT_HOLD_REASON_NOTE.md`.
If you need the even shorter version of that same hold cue for compact cron output, open `docs/CLI_SUMMARY_FIVE_REPO_COMMIT_HOLD_SHORT_REASON_NOTE.md`.
...
If you need the matching one-line maintainer cue after that small validation loop, open `docs/CLI_SUMMARY_VALIDATE_SMALL_SLICE_STATUS_LINE_NOTE.md`.
...
If you need the matching maintainer reminder for download-oriented JSON/Markdown/HTML summary paths, open `docs/CLI_SUMMARY_DOWNLOAD_NOTE.md`.
...
If you need a compact maintainer note for why the selected next-focus queue wins the handoff, open `docs/CLI_SUMMARY_NEXT_FOCUS_ADVANTAGE_NOTE.md`.
...
If you need a compact reviewer cue for pairing the selected queue with the saved JSON/Markdown/HTML summary bundle before a short status post, open `docs/CLI_SUMMARY_REPORT_REVIEW_QUEUE_NOTE.md`.
...
If you need a compact owner-ready note that keeps the dominant reviewer queue, queue share, and saved report bundle path in one replayable status line, open `examples/reviewer_queue_share_bundle_owner_status_line.md`.
...
PR-comment and markdown summaries now also expose `Reviewer queue follow-up priority summary`, so reviewers can reuse one deterministic `key -> key` triage string without reopening JSON.
...
If you need a compact reviewer note that keeps queue-share percentage visible beside the winning lane label, open `examples/reviewer_queue_priority_rank_queue_share_status.md`.
...
If you need a one-screen example for posting short validated progress against the same reviewer-queue report bundle, open `examples/reviewer_queue_report_bundle_progress_note.md`.
...
If you need a compact automation note for a JSON-friendly `proof_validation_command` alias of the same first-proof gate, open `docs/CLI_SUMMARY_PROOF_VALIDATION_ALIAS_NOTE.md`.
...
If you need a compact triage reminder for keeping one command, one status line, and one saved artifact path together during reruns, open `docs/CLI_SUMMARY_TRIAGE_RECHECK_NOTE.md`.
...
If you need a compact reminder to recheck the same fixed five-repo order before writing the short report, open `docs/CLI_SUMMARY_FIVE_REPO_RECHECK_ORDER_NOTE.md`.
