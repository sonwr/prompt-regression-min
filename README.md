...
PR-comment and markdown summaries now also expose `Reviewer queue next-focus case count`, `Reviewer queue next-focus active-case rate`, and `Reviewer queue next-focus source-case rate`, so the first rerun bucket can be sized in absolute cases as well as against both the active review scope and the original dataset without opening JSON.
