...
The JSON `summary.reviewer_queue` payload now mirrors that same handoff via explicit `next_focus_key`, `next_focus_label`, `next_focus_ids`, `next_focus_case_count`, `next_focus_queue_share`, and `next_focus_tie_mode` aliases, so bots can route and size the first rerun lane without reverse-engineering the larger queue object.
