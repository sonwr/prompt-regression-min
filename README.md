...
The JSON `summary.reviewer_queue` payload now mirrors that same handoff via explicit `next_focus_key`, `next_focus_label`, `next_focus_ids`, `next_focus_case_count`, and `next_focus_tie_mode` aliases, so bots can route the first rerun lane without reverse-engineering the larger queue object.
