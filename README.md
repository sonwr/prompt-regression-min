...
Need to pipe the reviewer note directly into CI or a PR bot? `--summary-pr-comment -` now shares the same stdout ergonomics as `--summary-markdown -`, so workflows can render or post the ready-to-paste note without creating a temp file first. For a copy-paste CI example, open `examples/ci_pr_comment_stdout.md`.
...
PR-comment and markdown summaries now also expose `Reviewer queue next-focus active-case rate` and `Reviewer queue next-focus source-case rate`, so the first rerun bucket can be sized against both the active review scope and the original dataset without opening JSON.
