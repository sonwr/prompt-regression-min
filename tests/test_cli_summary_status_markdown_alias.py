...
class CliSummaryStatusMarkdownAliasTests(unittest.TestCase):
    def test_cli_accepts_summary_status_md_alias(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            dataset = tmp / "dataset.jsonl"
            baseline = tmp / "baseline.jsonl"
            candidate = tmp / "candidate.jsonl"
            summary_md = tmp / "summary-status.md"
            ...
            subprocess.run(
                [
                    ...
                    "--summary-status-md", str(summary_md),
                    "--quiet",
                ],
                cwd=ROOT,
