...
class CliSummaryPrMarkdownAliasTests(unittest.TestCase):
    def test_cli_accepts_summary_pr_md_alias(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            dataset = tmp / "dataset.jsonl"
            baseline = tmp / "baseline.jsonl"
            candidate = tmp / "candidate.jsonl"
            summary_md = tmp / "summary-pr.md"
